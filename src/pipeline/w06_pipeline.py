...
def build_dims_facts(con: duckdb.DuckDBPyConnection) -> None:
    ...
    NOTE:
    If an older version created foreign keys, DuckDB can refuse DROP on referenced tables.
    Therefore: DROP dependent tables FIRST.
    """
    require_table(con, "silver_planet", "Run --mode silver first (or --mode all).")
    ...
    join_rows = con.execute("SELECT COUNT(*) FROM fact_planet_sk").fetchone()[0]
...
def build_gold(con: duckdb.DuckDBPyConnection) -> None:
    require_table(con, "fact_planet_sk", "Run --mode dims first (or --mode all).")
...
def export_artifacts(con: duckdb.DuckDBPyConnection, artifacts_dir: Path) -> None:
    ...
    if not (has1 and has2):
        raise RuntimeError("Missing gold views. Run --mode gold first (or --mode all).")

    out1 = artifacts_dir / "gold_by_discoverymethod.csv"
    out2 = artifacts_dir / "gold_by_host.csv"

    con.execute(f"COPY (SELECT * FROM gold_by_discoverymethod) TO {sql_path(out1)} (HEADER, DELIMITER ',')")
    con.execute(f"COPY (SELECT * FROM gold_by_host) TO {sql_path(out2)} (HEADER, DELIMITER ',')")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="W06A — DuckDB pipeline runner (idempotent)")
    p.add_argument("--project-root", default=".", help="Project root (contains data/, docs/, artifacts/)")
    p.add_argument("--db-path", default="data/exoplanets.duckdb", help="DuckDB path relative to project root")
    p.add_argument("--raw-csv", default="data/raw/pscomppars.csv", help="Raw CSV path relative to project root")
    p.add_argument("--mode", choices=["silver", "dims", "gold", "export", "all"], default="all")
